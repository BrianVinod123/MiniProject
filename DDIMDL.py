...
#import Dense, Dropout, Input, Activation, BatchNormalization
...
def evaluate(pred_type, pred_score, y_test, event_num, set_name):
    ...
    each_eval_type = 6
    result_eve = np.zeros((event_num, each_eval_type), dtype=float)
    y_one_hot = label_binarize(y_test, classes=np.arange(event_num))  # Correct usage of label_binarize
    pred_one_hot = label_binarize(pred_type, classes=np.arange(event_num))  # Correct usage of label_binarize
    ...
    result_all[7] = precision_score(y_test, pred_type, average='micro',zero_division='warn')
    result_all[8] = precision_score(y_test, pred_type, average='macro',zero_division='warn')
    ...
    for i in range(event_num):
        ...
        result_eve[i, 2] = roc_auc_score(y_one_hot[:, i], pred_one_hot[:, i], average=None)
