...
def NLPProcess(druglist, df_interaction):
    def addMechanism(node, sons, sonsNum, dependency, drugA, drugB, quene, i):
        if int(sonsNum[int(node-1)]) == 0:
            ...
            for k in sons[node-1]:
                ...
                if dependency[int(k - 1)].text == drugA[i] or dependency[int(k - 1)].text == drugB[i]:
                    continue
                quene.append(int(k))
                quene = addMechanism(int(k), sons, sonsNum, dependency, drugA, drugB, quene, i)
        return quene
    ...
    for i in range(len(event)):
        ...
        if root is not None:
            ...
            if quene:
                quene = addMechanism(quene[0], sons, sonsNum, dependency, drugA, drugB, [], i)
            quene.sort()
            
            mechanism_text = " ".join(dependency[j-1].text for j in quene)
            mechanism.append(mechanism_text)
